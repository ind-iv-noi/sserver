...
void create_directory(std::string s)
{
	namespace fs = std::experimental::filesystem;
	fs::create_directories(s + "/new");
	fs::create_directories(s + "/old");
}
void create_map() {
	std::string u, p;
	while (read_file >> u >> p >> std::ws)
		user_password[u] = p,create_directory(u);
}
...
void add_message(std::string sender, std::string reciever,std::string message) {
	
}

void func(SOCKET cli) {
	char read_buff[BUFF_LEN],user[BUFF_LEN],pass[BUFF_LEN],send_buff[BUFF_LEN],reciever[BUFF_LEN];
	...
	do {
		...
		if (iResult > 0) {
			...
			if (!logged_in) {
				...
				if (read_buff[0] == 'N') {
					while (1) {
						if (user_password.find(std::string(read_buff + 1)) == user_password.end())
							sprintf_s(send_buff, "0\0");
						else
							sprintf_s(send_buff, "1\0");
						iResult = send(cli, send_buff, BUFF_LEN, 0);
						if (send_buff[0] == '1')
							break;
						iResult = recv(cli, read_buff, BUFF_LEN, 0);
					}
					if (send_buff[0] == '0')
						break;
					for (int i = 1; i < BUFF_LEN && read_buff[i] != '\0'; i++)
						reciever[i - 1] = read_buff[i];
					do {
						iResult = recv(cli, read_buff, BUFF_LEN, 0);
						std::cout << iResult << " " << read_buff << '\n';
						add_message(user, reciever, read_buff+1);
					} while (read_buff[0] != '~');
					std::cout << "Am iesit din msj\n";
				}
			}
		}
		else
